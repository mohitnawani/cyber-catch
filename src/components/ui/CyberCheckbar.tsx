import { CalendarDays, TableProperties, Box } from 'lucide-react'
...
export default function CyberCheckbar() {
  return (
    <aside className="-my-3 w-[178px] shrink-0 bg-white-pure px-4 py-12 shadow-[2px_0_12px_rgba(15,41,64,0.04)]">
      <div className="flex h-10 w-10 items-center justify-center rounded-full text-[#604BFF]">
        <img src={tick} alt="Cyber Check" className="h-10 w-auto border-accent" />
      </div>
      <h1 className="mt-3 text-[15px] font-bold leading-[1.05] text-navy">
        Welcome to<br />Cyber Check!
      </h1>
      <nav className="mt-6 space-y-4">
        {steps.map(({ label, to, icon: Icon }) => (
          <NavLink
            ...
            to={to}
            className={({ isActive }) =>
              `flex items-center gap-2 rounded-sm px-2 py-2 text-xs font-medium transition ${
