...
export default function UploadUsersPage({
  ...
  return (
    <section className="max-w-[920px] pt-1 mx-auto">
      ...
      <div className="mt-5 flex items-center gap-4 rounded-xl bg-white px-5 py-4 shadow-[0_6px_20px_rgba(15,41,64,0.07)]">
      ...
      <h3 className="mt-7 text-base font-bold text-black">Import Members</h3>
      <div
        onClick={openPicker}
        className="mt-3 flex min-h-[270px] cursor-pointer flex-col items-center justify-center rounded-md border-2 border-dashed border-[#d8d9ea] bg-white/35 px-6 text-center transition hover:border-brand/50 hover:bg-white/60"
      >
        <input
          ref={fileInput}
          onChange={chooseFile}
          ...
          className="hidden"
        />
        <Upload size={24} className="mb-4 text-brand" />
        <h4 className="text-sm font-bold text-black">
          {fileName || "Drag & Drop The Excel File Here"}
        </h4>
        <p className="mt-2 text-[10px] text-navy/65">
        ...
        <button
          ...
          className={`${buttonClass} mt-4 h-8 bg-brand px-4 text-[10px] text-white`}
        >
          Upload Member Excel
        </button>
        <p className="mt-4 text-[9px] text-navy/60">
