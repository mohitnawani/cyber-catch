...
export default function Users() {
  ...
  const openEdit = (user: ImportedMember) => {
    setTargetUser(user);
    setForm({
      name: user.name,
      email: user.email,
  ...
  const saveUser = () => {
    const user: ImportedMember = {
      id: targetUser?.id ?? Date.now(),
      name: form.name.trim() || "New User",
      email: form.email.trim() || "new.user@company.com",
      department: form.department.trim() || "General",
      role: form.role.trim() || "Member",
    };
    setActiveUsers((current) =>
      targetUser
        ? current.map((item) => (item.id === targetUser.id ? user : item))
        : [...current, user],
    );
    setModal(null);
  };
  const archiveUser = () => {
    ...
    setArchivedUsers((current) => [...current, targetUser]);
    setTargetUser(null);
    setModal(null);
  };
  const restoreUser = () => {
    ...
    setActiveUsers((current) => [...current, targetUser]);
    setTargetUser(null);
    setModal(null);
  };
  ...
  const saveGroup = () => {
    const name = form.name.trim() || "New Group";
    setGroups((current) =>
      current.includes(groupName)
        ? current.map((group) => (group === groupName ? name : group))
        : [...current, name],
    );
    setGroupName(name);
    setModal(null);
  ...
  const archiveGroup = () => {
    setGroups((current) => current.filter((group) => group !== groupName));
    setModal(null);
    setPage("groups");
  };
  ...
  const removeMember = (id: number) => {
    const user = groupMembers.find((member) => member.id === id) ?? null;
    setTargetMember(user);
    setModal("remove");
  };
  const confirmRemoveMember = () => {
    if (targetMember)
      setGroupMembers((current) =>
        current.filter((member) => member.id !== targetMember.id),
      );
  ...
  if (page === "users")
    content = (
      <UserManagementPage
        users={activeUsers}
        archived={false}
        onImport={() => setPage("start")}
        onAdd={openAdd}
        onGroups={() => setPage("groups")}
        onArchiveList={() => setPage("archived")}
        onActiveList={() => setPage("users")}
        onArchive={(id) => {
          const user = activeUsers.find((item) => item.id === id) ?? null;
          setTargetUser(user);
          setModal("archive");
        }}
        onRestore={() => {}}
    ...
    content = (
      <UserManagementPage
        ...
        onArchive={() => {}}
        onRestore={(id) => {
          const user = archivedUsers.find((item) => item.id === id) ?? null;
          setTargetUser(user);
          setModal("restore");
        }}
        onEdit={openEdit}
    ...
    content = (
      <GroupsPage
        ...
        onArchive={(name) => {
          setGroupName(name);
          setModal("group-archive");
        }}
        onEdit={openGroupEdit}
      />
    );
  else if (page === "group-details")
    content = (
      <GroupDetailsPage
        groupName={groupName}
        members={groupMembers}
        onBack={() => setPage("groups")}
        onAddMembers={() => setModal("add-members")}
        onRemove={removeMember}
      />
    );
  else if (page === "start")
    content = <ImportUsersPage onImport={() => setPage("upload")} />;
  else if (page === "upload")
    content = (
      <UploadUsersPage
        fileName={fileName}
    ...
    <>
      ...
      {modal && (
        <FlowModal
          ...
          onSave={saveUser}
          onArchive={modal === "group-archive" ? archiveGroup : archiveUser}
...
function FlowModal({
  ...
  if (type === "group-archive")
    return (
      <UserModal
        ...
        compact
      >
        <p className="mt-5 text-xs text-navy/75">
          Are you sure you want to archive this group?
        </p>
      </UserModal>
    );
  if (type === "archive" || type === "restore" || type === "remove") {
    const action =
      type === "restore" ? "Restore" : type === "remove" ? "Remove" : "Archive";
    return (
      <UserModal
        title={`${action} User`}
        confirmText="Yes"
  ...
  if (type === "add-members")
    return <AddMembersModal onClose={onClose} onAddMembers={onAddMembers} />;
  if (type === "group-add" || type === "group-edit")
    return (
      <UserModal
        title={type === "group-add" ? "Add Group" : "Update Group"}
        confirmText={type === "group-add" ? "Submit" : "Update"}
        onClose={onClose}
        onConfirm={onSaveGroup}
      >
        <p className="mt-3 text-xs text-navy/75">
          Enter the data for the group you want to add to the system.
        </p>
  ...
  return (
    <UserModal
      title={editing ? "Update User" : "Add New Members"}
...
function AddMembersModal({
  ...
  return (
    <UserModal
      ...
      onClose={onClose}
      onConfirm={() => onAddMembers(selectedIds)}
      compact
    >
      <p className="mt-3 text-xs text-navy/75">
        Add one or more users to this group by selecting them from the list.
      </p>
      <div className="mt-4 flex h-8 items-center rounded-full bg-[#eff1ff] px-3 text-[10px] text-[#8b8daf]">
      ...
      <div className="mt-3 max-h-64 space-y-2 overflow-y-auto">
        {members.map((member) => (
          <label
            key={member.id}
            className="flex cursor-pointer items-center gap-3 rounded-lg bg-[#fafafa] p-2 text-[10px] shadow-[0_4px_12px_rgba(15,41,64,0.04)]"
          >
            <input
              type="checkbox"
            ...
            />
            <span className="flex h-7 w-7 items-center justify-center rounded-full bg-[#f1d1bd]">
              {member.name[0]}
            </span>
            <span>
