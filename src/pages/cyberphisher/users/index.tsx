...
function FlowModal({ type, form, onChange, onClose, onSave, onArchive, onRestore, onSaveGroup, onAddMembers, onRemoveMember }: { type: Exclude<Modal, null>; form: Form; onChange: (key: keyof Form, value: string) => void; onClose: () => void; onSave: () => void; onArchive: () => void; onRestore: () => void; onSaveGroup: () => void; onAddMembers: (ids: number[]) => void; onRemoveMember: () => void }) {
  if (type === 'group-archive') return <UserModal title="Archive Group" confirmText="Yes" onClose={onClose} onConfirm={onArchive} compact><p className="mt-5 text-xs text-navy/75">Are you sure you want to archive this group?</p></UserModal>
  if (type === 'archive' || type === 'restore' || type === 'remove') { const action = type === 'restore' ? 'Restore' : type === 'remove' ? 'Remove' : 'Archive'; return <UserModal title={`${action} User`} confirmText="Yes" onClose={onClose} onConfirm={type === 'archive' ? onArchive : type === 'restore' ? onRestore : onRemoveMember} compact><p className="mt-5 text-xs text-navy/75">Are you sure you want to {action.toLowerCase()} this user?</p></UserModal> }
  if (type === 'add-members') return <AddMembersModal onClose={onClose} onAddMembers={onAddMembers} />
  if (type === 'group-add' || type === 'group-edit') return <UserModal title={type === 'group-add' ? 'Add Group' : 'Update Group'} confirmText={type === 'group-add' ? 'Submit' : 'Update'} onClose={onClose} onConfirm={onSaveGroup}><p className="mt-3 text-xs text-navy/75">Enter the data for the group you want to add to the system.</p><div className="mt-5 grid grid-cols-2 gap-3"><TextInput label="Name" value={form.name} onChange={value => onChange('name', value)} /><TextInput label="Risk Level (Optional)" value={form.role} onChange={value => onChange('role', value)} /></div><label className="mt-4 block text-[10px] font-bold text-navy">Description<textarea value={form.department} onChange={event => onChange('department', event.target.value)} className="mt-1 block h-20 w-full resize-none rounded border border-[#e5e6ef] p-2 text-[10px] font-normal outline-none focus:border-brand" /></label></UserModal>
...
function AddMembersModal({ onClose, onAddMembers }: { onClose: () => void; onAddMembers: (ids: number[]) => void }) {
  ...
  return <UserModal title="Add Users" confirmText="Save & Close" onClose={onClose} onConfirm={() => onAddMembers(selectedIds)} compact><p className="mt-3 text-xs text-navy/75">Add one or more users to this group by selecting them from the list.</p><div className="mt-4 flex h-8 items-center rounded-full bg-[#eff1ff] px-3 text-[10px] text-[#8b8daf]"><Search size={12} className="mr-2" />Input</div><div className="mt-3 max-h-64 space-y-2 overflow-y-auto">{members.map(member => <label key={member.id} className="flex cursor-pointer items-center gap-3 rounded-lg bg-[#fafafa] p-2 text-[10px] shadow-[0_4px_12px_rgba(15,41,64,0.04)]"><input type="checkbox" checked={selectedIds.includes(member.id)} onChange={() => toggle(member.id)} className="accent-brand" /><span className="flex h-7 w-7 items-center justify-center rounded-full bg-[#f1d1bd]">{member.name[0]}</span><span><b className="block">{member.name}</b>{member.email}</span></label>)}</div></UserModal>
}
