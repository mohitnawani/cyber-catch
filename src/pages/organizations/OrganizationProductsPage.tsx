...
export default function OrganizationProductsPage() {
  ...
  const [activeTab, setActiveTab] = useState<"general" | "products" | "members" | "subsidiaries">("general");
  ...
  return (
    <section className="max-w-230 pt-1">
      ...
      <div className="mt-3 flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <nav className="mt-4 flex gap-5 text-[10px] font-semibold text-navy/60">
            <button onClick={() => setActiveTab("general")} className={activeTab === "general" ? "border-b-2 border-brand pb-2 text-brand" : "pb-2"}>
              General
            </button>
            <button onClick={() => setActiveTab("products")} className={activeTab === "products" ? "border-b-2 border-brand pb-2 text-brand" : "pb-2"}>Products</button>
            <button onClick={() => setActiveTab("members")} className={activeTab === "members" ? "border-b-2 border-brand pb-2 text-brand" : "pb-2"}>Members</button>
            <button onClick={() => setActiveTab("subsidiaries")} className={activeTab === "subsidiaries" ? "border-b-2 border-brand pb-2 text-brand" : "pb-2"}>Subsidiaries</button>
          </nav>
        </div>
        <div className="flex gap-2">{(activeTab === "general" || activeTab === "subsidiaries") && <button onClick={() => setIsChildModalOpen(true)} className={`${buttonClass} bg-brand text-white`}>Add Child Organization</button>}{(activeTab === "general" || activeTab === "subsidiaries" || activeTab === "members") && <button className={`${buttonClass} bg-brand text-white`}>Add Members</button>}{activeTab === "products" && <button className={`${buttonClass} bg-brand text-white`}>
          <Plus size={13} className="mr-1" />
          Add New Product
        </button>}{activeTab === "members" && <button className={`${buttonClass} bg-brand text-white`}>Add Members</button>}</div>
      </div>
      {activeTab === "general" && <div className="mt-5"><OrganizationGeneralPage /><div className="mt-8"><h2 className="text-base font-bold text-black">Child Organizations</h2><div className="mt-3 space-y-2">{children.map((child, index) => <div key={`${child}-${index}`} className="flex items-center justify-between rounded-xl bg-white px-4 py-3 text-[10px] text-navy/75 shadow-[0_4px_14px_rgba(15,41,64,0.05)]"><span>{child}</span><button className="text-[#8586a3]">•••</button></div>)}</div></div></div>}
      {activeTab === "products" && <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
      ...
      {activeTab === "subsidiaries" && <OrganizationSubsidiariesPage children={children} onManage={() => setActiveTab("general")} />}
      {pendingProduct && (
      ...
      {isChildModalOpen && <AddChildOrganizationModal parentName={organizationTitle} onClose={() => setIsChildModalOpen(false)} onSave={saveChildOrganization} />}
    </section>
  );
}
